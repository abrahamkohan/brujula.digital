...
import { Search, Shield, Loader2, AlertCircle, TrendingUp, FileText, Activity } from "lucide-react";
...
interface EmpresaScore {
  ...
  contratos_12m: number;
  nivel_actividad: string;
  contratos: Array<{ ocid: string; date: string; tags: string[] }>;
}
    ...
    <div className="min-h-screen bg-[#F5F4ED] font-sans">
      <section className="max-w-2xl mx-auto px-4 sm:px-6 pt-16 sm:pt-20 pb-8 sm:pb-12 text-center">
        <div className="inline-flex items-center gap-2 bg-white border border-[#D4D2C9] text-[#87867F] text-[10px] sm:text-xs px-3 sm:px-4 py-1.5 rounded-full mb-4 sm:mb-6 font-medium">
          <Shield className="h-3.5 w-3.5 text-[#C96442]" /> KYC · Contrataciones públicas
        </div>
        ...
        <p className="text-base sm:text-lg text-[#5C5B57] max-w-md mx-auto leading-relaxed">
          Perfil de contrataciones de una empresa por RUC. Contratos con el Estado, nivel de actividad, categorías.
        </p>
      </section>
          ...
          <CardContent className="pt-6 space-y-4">
            <div className="flex flex-col sm:flex-row gap-2">
              <Input
                ...
                autoFocus
              />
              <Button onClick={buscar} disabled={loading || ruc.length < 8} className="h-12 px-6 bg-[#C96442] hover:bg-[#B5583A] text-white font-semibold">
            ...
            {data && (
              <div className="space-y-4 pt-2">
                  ...
                  {data.categorias.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      {data.categorias.map((c, i) => (
                        <Badge key={i} className="bg-white text-[#5C5B57] hover:bg-white border-0 text-xs">{c}</Badge>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: "Contratos totales", value: data.contratos_total, icon: FileText, color: "text-[#C96442]" },
                    { label: "Últimos 12 meses", value: data.contratos_12m, icon: TrendingUp, color: "text-[#5A7D5A]" },
                    { label: "Nivel actividad", value: data.nivel_actividad, icon: Activity, color: data.nivel_actividad === "Alto" ? "text-[#5A7D5A]" : data.nivel_actividad === "Medio" ? "text-[#B89B4B]" : "text-[#87867F]" },
                  ].map(s => (
                ...
                {data.contratos.length > 0 && (
                  <>
                    <p className="text-xs font-semibold text-[#87867F] uppercase tracking-wider">Últimos contratos</p>
                    {data.contratos.map((c, i) => (
                      <div key={i} className="bg-white rounded-xl border border-[#D4D2C9] p-3 flex items-center justify-between">
                        <div className="min-w-0">
                          <p className="text-xs text-[#5C5B57] truncate max-w-[200px]">{c.ocid}</p>
                          <p className="text-[10px] text-[#87867F]">{c.date?.slice(0, 10)}</p>
                        </div>
                        <div className="flex gap-1 shrink-0">
                          {c.tags?.map((t, j) => (
                            <Badge key={j} className="bg-[#F5F4ED] text-[#87867F] hover:bg-[#F5F4ED] border-0 text-[10px]">{t}</Badge>
                          ))}
                        </div>
                      </div>
                    ))}
