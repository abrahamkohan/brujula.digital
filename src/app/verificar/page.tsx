      ...
      <section className="max-w-2xl mx-auto px-4 sm:px-6 pb-16">
        ...
        {modo==="bulk" && (
          <Card className="border-[#D4D2C9] shadow-sm"><CardContent className="pt-6 space-y-4">
            ...
            {bulkResults.length>0&&<div className="overflow-x-auto">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs text-[#87867F]">{bulkResults.length} resultados</p>
                <button onClick={()=>{
                  const csv = "CI,Nombre,Organismo,Cargo,Estado\n"+bulkResults.map(r=>`${r.ci},"${r.nombre}","${r.organismo}","${r.cargo}",${r.estado||"no"}`).join("\n");
                  const b = new Blob([csv],{type:"text/csv"});
                  const a = document.createElement("a"); a.href=URL.createObjectURL(b); a.download="brujula-bulk-"+Date.now()+".csv"; a.click();
                }} className="text-xs text-[#C96442] hover:underline font-medium">Descargar CSV ↗</button>
              </div>
