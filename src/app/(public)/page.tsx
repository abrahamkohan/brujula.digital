...
const PORTADA_TIPOS = [
  { tipo: "gastronomia", label: "Restaurantes" },
  { tipo: "bar",         label: "Bares" },
  { tipo: "shopping",    label: "Shoppings" },
  { tipo: "complejo",    label: "Escapadas" },
];
    ...
    <div className="min-h-screen bg-[#F5F4ED]">
        ...
        <div className="bg-white rounded-2xl border border-[#D4D2C9] shadow-lg overflow-hidden">
          <div className="flex flex-col sm:flex-row items-stretch divide-y sm:divide-y-0 sm:divide-x divide-[#D4D2C9]/60">
            ...
            <div className="flex-1 px-5 py-3.5 flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs text-[#87867F] ml-1">Desde USD 5</span>
            </div>
            <div className="px-5 py-3.5 flex items-center gap-2.5">
              <Car className="h-4 w-4 text-[#87867F] shrink-0" />
              <p className="text-xs text-[#5C5B57]">Taxi oficial · En la terminal</p>
            </div>
            <div className="px-5 py-3.5 flex items-center gap-2.5">
              <Bus className="h-4 w-4 text-[#87867F] shrink-0" />
              <p className="text-xs text-[#5C5B57]">Línea 30 · La más económica</p>
            </div>
            <Link href="/como-llegar" className="shrink-0 flex items-center gap-1.5 px-5 py-4 sm:py-0 text-xs font-semibold text-[#C96442] hover:bg-[#C96442]/5 transition-colors whitespace-nowrap">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-14 py-12">
        ...
        <section>
          <div className="flex items-center gap-3 mb-5">
            <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">¿Dónde quedarse?</h2>
              ...
              <Link key={z.id} href={`/guia/hotel?zona=${z.id}`} className="group bg-white rounded-2xl border border-[#D4D2C9] p-5 hover:border-[#C96442]/30 hover:shadow-md transition-all space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-[#87867F] mt-0.5">{z.sub}</p>
                  </div>
                  <span className="shrink-0 flex items-center gap-1 text-[10px] font-medium bg-[#F5F4ED] text-[#5C5B57] px-2 py-1 rounded-full whitespace-nowrap">
                ...
                {z.hoteles.length > 0 && (
                  <ul className="space-y-1">
                    {z.hoteles.map((h) => (
                      <li key={h.id} className="text-xs text-[#5C5B57] flex items-center gap-1.5">
                        <span className="w-1 h-1 rounded-full bg-[#D4D2C9] shrink-0" />{h.name}
                      </li>
                    ))}
                  </ul>
                )}
                <p className="text-xs font-medium text-[#C96442] flex items-center gap-1">{z.total} hoteles disponibles <ArrowRight className="h-3 w-3" /></p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center gap-3 mb-5">
            <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">¿Qué hacer en Asunción?</h2>
            <div className="flex-1 h-px bg-[#D4D2C9]/50" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {PORTADA_TIPOS.map(({ tipo, label }) => {
              ...
              return (
                <Link key={tipo} href={`/guia/${tipo}`} className="group relative rounded-2xl overflow-hidden aspect-[4/3] block shadow-sm hover:shadow-xl transition-all hover:scale-[1.02]">
                  {imagen ? (
          ...
          <div className="mt-4 text-center">
            <Link href="/guia" className="inline-flex items-center gap-1.5 text-sm text-[#C96442] hover:underline font-medium">
        ...
        {destacados.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">Destacados</h2>
              <div className="flex-1 h-px bg-[#D4D2C9]/50" />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {destacados.map((item) => (
                <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer" className="group relative rounded-2xl overflow-hidden aspect-[4/3] block shadow-sm hover:shadow-xl transition-all hover:scale-[1.02]">
                  {item.image ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                  {item.badge && (
                    <span className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-medium bg-black/50 backdrop-blur-sm text-white">{item.badge}</span>
                  )}
                  <div className="absolute bottom-0 left-0 right-0 p-4">
        ...
        {eventos.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-6">
              <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">Eventos próximos</h2>
              <div className="flex-1 h-px bg-[#D4D2C9]/50" />
              <Link href="/eventos" className="text-xs text-[#C96442] hover:underline flex items-center gap-1 shrink-0">Ver todos <ArrowRight className="h-3 w-3" /></Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {eventos.map((e) => (
                <EventCard key={e.id} event={{ id: e.id, titulo: e.titulo, fecha: e.fecha, venue: e.venue ?? "", categoria: e.categoria, image_url: e.image_url }} featured href={`/eventos/evento/${e.id}`} />
              ))}
            </div>
          </section>
        )}
        ...
        {itinerarios.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-5">
              <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">Itinerarios destacados</h2>
              <div className="flex-1 h-px bg-[#D4D2C9]/50" />
              <Link href="/itinerarios" className="text-xs text-[#C96442] hover:underline flex items-center gap-1 shrink-0">Ver todos <ArrowRight className="h-3 w-3" /></Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {itinerarios.map((it: any) => (
                <Link key={it.id} href={`/itinerarios/${it.slug}`} className="group bg-white rounded-2xl border border-[#D4D2C9] overflow-hidden shadow-sm hover:shadow-lg hover:border-[#C96442]/30 transition-all">
                  <div className="aspect-[16/9] bg-[#1F1E1D] overflow-hidden relative">
                    {it.imagen ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-[#1F1E1D] to-[#2A2825] flex items-center justify-center"><Compass className="h-8 w-8 text-white/20" /></div>
                    )}
                    {it.tipo && (
        ...
        <section>
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 h-px bg-[#D4D2C9]/50" />
            <h2 className="font-[family-name:var(--font-heading)] text-xl sm:text-2xl font-bold text-[#1F1E1D] tracking-tight">Explorá por zona</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
            {zonasConData.slice(0, 8).map((z) => {
              ...
              return (
                <Link key={z.id} href={`/zona/${z.id}`} className="group relative rounded-2xl overflow-hidden aspect-[4/3] block shadow-sm hover:shadow-xl transition-all hover:scale-[1.02]">
                  {hero ? (
          ...
          {zonasConData.length > 8 && (
            <div className="flex flex-wrap gap-2">
              {zonasConData.slice(8).map((z) => (
                <Link key={z.id} href={`/zona/${z.id}`} className="inline-flex items-center px-4 py-2 rounded-full bg-white border border-[#D4D2C9] text-sm text-[#5C5B57] hover:border-[#C96442] transition-all">
                  📍 {z.label}
                </Link>
              ))}
            </div>
      ...
      <footer className="border-t border-[#D4D2C9] py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-[#87867F]">
          <div>
            <span className="font-bold text-[#1F1E1D]">Bru<span className="text-[#C96442] italic">jula</span></span>
            <span className="ml-2">· Guía de Asunción</span>
          </div>
          <div className="flex items-center gap-4">
            <a href="https://kohancampos.com.py" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-[#5C5B57] hover:text-[#1F1E1D] transition-colors">Kohan &amp; Campos</a>
