...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F4ED]">
      ...
      {/* ── CÓMO LLEGAR — teaser ─────────────────────────── */}
      <div className="max-w-6xl mx-auto px-4 sm:px-6 -mt-5 relative z-10">
        <div className="bg-white rounded-2xl border border-[#D4D2C9] shadow-sm overflow-hidden">
          <div className="flex flex-col sm:flex-row items-stretch">
            {/* Label */}
            <div className="shrink-0 px-5 py-4 sm:py-0 sm:flex sm:items-center border-b sm:border-b-0 sm:border-r border-[#D4D2C9]/60">
              <p className="text-xs font-semibold uppercase tracking-widest text-[#87867F]">
                Desde el aeropuerto
              </p>
            </div>

            {/* Opciones */}
            <div className="flex-1 grid grid-cols-3 divide-x divide-[#D4D2C9]/60">
              <div className="px-4 py-3 flex items-center gap-2.5">
                <Smartphone className="h-4 w-4 text-[#C96442] shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-[#1F1E1D]">Uber · Bolt</p>
                  <p className="text-[11px] text-[#87867F]">Desde USD 5</p>
                </div>
              </div>
              <div className="px-4 py-3 flex items-center gap-2.5">
                <Car className="h-4 w-4 text-[#5C5B57] shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-[#1F1E1D]">Taxi oficial</p>
                  <p className="text-[11px] text-[#87867F]">En la terminal</p>
                </div>
              </div>
              <div className="px-4 py-3 flex items-center gap-2.5">
                <Bus className="h-4 w-4 text-[#5C5B57] shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-[#1F1E1D]">Línea 30</p>
                  <p className="text-[11px] text-[#87867F]">La más económica</p>
                </div>
              </div>
            </div>
            ...
            <Link
              href="/como-llegar"
              className="shrink-0 flex items-center gap-1.5 px-5 py-4 sm:py-0 border-t sm:border-t-0 sm:border-l border-[#D4D2C9]/60 text-xs font-semibold text-[#C96442] hover:bg-[#C96442]/5 transition-colors whitespace-nowrap"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {hotelesPorZona.map((z) => (
              <Link
                key={z.id}
                href={`/zona/${z.id}`}
                className="group bg-white rounded-2xl border border-[#D4D2C9] p-5 hover:border-[#C96442]/30 hover:shadow-md transition-all space-y-3"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-[#1F1E1D] group-hover:text-[#C96442] transition-colors">
                      {z.label}
                    </h3>
                  ...
                  <ul className="space-y-1">
                    {z.hoteles.map((h) => (
                      <li key={h.id} className="text-xs text-[#5C5B57] flex items-center gap-1.5">
                        <span className="w-1 h-1 rounded-full bg-[#D4D2C9] shrink-0" />
                        {h.name}
                      </li>
                    ))}
                  </ul>
                ...
                <p className="text-xs font-medium text-[#C96442] flex items-center gap-1">
